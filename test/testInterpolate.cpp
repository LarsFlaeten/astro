...
{
    PosState s1;
    s1.r = Vec3(0.0, 0.0, 0.0);
    s1.v = Vec3(1.0, 2.0, 3.0);
    EphemerisTime et1(1000);

    PosState s2;
    s2.r = Vec3(1.0, 2.0, 3.0);
    s2.v = Vec3(0.0, 0.0, 0.0);
    EphemerisTime et2(1001);

    PosState s3;
    astro::hermite(s1, et1, s2, et2, et1, s3);
    ASSERT_EQ(s3.r, Vec3(0.0, 0.0, 0.0));
    ASSERT_EQ(s3.v, Vec3(1.0, 2.0, 3.0));

    astro::hermite(s1, et1, s2, et2, et2, s3);
    ASSERT_EQ(s3.r, Vec3(1.0, 2.0, 3.0));
    ASSERT_EQ(s3.v, Vec3(0.0, 0.0, 0.0));
...
TEST_F(InterpolateTest, Evolution)
{
    PosState s1;
    s1.r = Vec3(0.0, 0.0, 0.0);
    s1.v = Vec3(2.0, 4.0, 6.0);
    PosState s2;
    s2.r = Vec3(1000.0, 2000.0, 3000.0);
    s2.v = Vec3(0.0, 0.0, 0.0);
     
    PosState s3;
    astro::hermite(s1, EphemerisTime(0), s2, EphemerisTime(1000), EphemerisTime(500), s3);

    ASSERT_EQ(s3.v, Vec3(1, 2, 3));
    ASSERT_EQ(s3.r, Vec3(1000*3/4, 2000*3/4, 3000*3/4));
...
TEST_F(InterpolateTest, OrbitState1)
{
    ...
    double r = 7283.46;
    state0.r = Vec3(r, 0.0 , 0.0);  //[km]
    state0.v = Vec3(0.0,1.2*v*sqrt(0.5), 0.5*v*sqrt(0.5));      //[km/s]
    ...
    for(double f = 0.02; f < 1.0; f+= 0.02)
    {
        ...
        ASSERT_LT(glm::length(state_integr.r - state_interp.r), 1.0E-5);
        ASSERT_LT(glm::length(state_integr.v - state_interp.v), 1.0E-5);
    }
